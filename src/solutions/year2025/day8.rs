...
pub fn part2(input: &str) -> impl std::fmt::Display + use<> {
    unsafe {
        ...
        let mut edges = BinaryHeap::with_capacity((999 * 999) >> 1);
        edges.extend((0..1000).flat_map(|i| {
            let a = points.get_unchecked(i);
            ((i + 1)..1000).map(move |j| {
                let b = points.get_unchecked(j);
                let dx = a.0 - b.0;
        ...
        // let mut edges = ((0..1000).flat_map(|i| {
        //     let a = points.get_unchecked(i);
        //     ((i + 1)..1000).map(move |j| {
        //         let b = points.get_unchecked(j);
        //         let dx = a.0 - b.0;
        ...
        // let mut edges = Vec::with_capacity((999 * 999) >> 1);
        // (0..1000).for_each(|i| {
        //     let a = points.get_unchecked(i);
        //     ((i + 1)..1000).for_each(|j| {
        //         let b = points.get_unchecked(j);
        ...
        for edge in edges.into_iter_sorted() {
            // for edge in edges {
            if union(edge.1, edge.2, &mut parent, &mut size)
                && size[find(edge.1, &mut parent)] == 1000
            {
